...
bool ready = false;
double curTime = 0.0;
vector3 prevCow(0,0,0);

vector3 makeBSpline(float t, vector3 p0, vector3 p1, vector3 p2, vector3 p3);
void BSpline(double time);
void makeDirection(vector3 pos);
...
void display()
{
    ...
    if(clickCnt == 6) {
        ...
        if(ready == false){
            ...
            ready = true;
            curTime = glfwGetTime() - animStartTime;
        }
        if(curTime< 18){
            curTime = glfwGetTime() - animStartTime;
            BSpline(curTime);
	...
	if (button == GLFW_MOUSE_BUTTON_LEFT)
	{
		...
		else if(state == GLFW_UP)
		{
            ...
            if(isInit){
                ...
                cowPosition[clickCnt] = cow2wld;
                cowVector[clickCnt] = vector3(cow2wld._14, cow2wld._24, cow2wld._34);
                
                p[clickCnt].x = cowPosition[clickCnt].getTranslation().x;
                p[clickCnt].y = cowPosition[clickCnt].getTranslation().y;
                p[clickCnt].z = cowPosition[clickCnt].getTranslation().z;
...
void onMouseDrag( int x, int y)
{
	...
	if (isDrag) 
	{
		...
		if (isDrag==V_DRAG && !isInit)
		{
            ...
            {
                Ray ray;
                screenCoordToRay(clickedPos, y, ray);
                PickInfo &pp=pickInfo;
...
vector3 makeBSpline(float t, vector3 p0, vector3 p1, vector3 p2, vector3 p3) {
	...
	vector3 a = (-p0 + 3 * p1 - 3 * p2 + p3) / 6.0f;
	vector3 b = (3 * p0 - 6 * p1 + 3 * p2) / 6.0f;
	vector3 c = (-3 * p0 + 3 * p2) / 6.0f;
	vector3 d = (p0 + 4 * p1 + p2) / 6.0f;

	vector3 pos = d + t * (c + t * (b + t * a));
...
void BSpline(double time) {
	...
	vector3 p0 = cowVector[(pos + 5) % 6];
	vector3 p1 = cowVector[pos];
	vector3 p2 = cowVector[(pos + 1) % 6];
	vector3 p3 = cowVector[(pos + 2) % 6];

	double t = time * 2 - (int)(time * 2);

	vector3 newPos = makeBSpline(t, p0, p1, p2, p3);

	makeDirection(newPos);
...
void makeDirection(vector3 pos) {
	...
	rotateY.setRotation(vector3(0, 1, 0), angle, true);
	rotateZ.setRotation(vector3(0, 0, 1), ang2, true);
	cowMatrix.mult(cowMatrix, rotateY);
	cowMatrix.mult(cowMatrix, rotateZ);
